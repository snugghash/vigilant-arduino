...
void serial_receive(void){
  ...
  if(!digitalRead(button1Pin)) {
    Serial.write("1 pressed\n");
    buttonState = '1';
  }
  if(!digitalRead(button2Pin)) {
    Serial.write("2 pressed\n");
    buttonState = '2';
  }
  if(!digitalRead(button3Pin)) {
    Serial.write("3 pressed\n");
    buttonState = '3';
  }
  ...
  // Joystick communication code
  int analogPosition = analogRead(joystickPinX);
  //Serial.println(analogPosition);
  if(analogPosition > 700) {
    Serial.println("HiGh");
    // Send corresponding message, 1-3
    SendPayload[0] = buttonState;
    //SendPayload[1] = 0;
    stringComplete = true;
  }
  else if(analogPosition < 300) {
    Serial.println("LoW");
    // Send 4-6
    SendPayload[0] = buttonState + 3;
    //SendPayload[1] = 0;
    stringComplete = true;
  }
